#!/usr/bin/env python3
"""
make_seed.py - generate the seed/ CSV files for the Team 1 trade database.

    python scripts/make_seed.py            # (re)write every file in seed/
    python scripts/make_seed.py --check    # fail if seed/ differs from what this would write
    python scripts/make_seed.py --stdout <table>   # print one file without writing

The dataset is written by hand here, not sampled randomly, so it is
deterministic: running this twice produces byte-identical files and a diff in
seed/ always means somebody changed something on purpose.

The point of generating rather than hand-editing CSVs is CONSISTENCY. The
portfolio rows are not typed in - they are COMPUTED by replaying every
successful order through the same settlement arithmetic the application layer
will use:

    DELIVERY fills  ->  portfolio_holding    (quantity never negative)
    INTRADAY fills  ->  portfolio_positions  (quantity may be negative = short)

so the seeded portfolio can never silently disagree with the seeded orders.
scripts/verify_db.py re-checks that agreement against the live database.

Coverage the dataset deliberately includes:
  * all three client states: ACTIVE, SUSPENDED, CLOSED
  * a delisted instrument that still has historical orders pointing at it
  * all four order states: RECEIVED, IN_PROGRESS, SUCCESS, FAILED
  * both product types, both sides
  * an intraday SHORT, so portfolio_positions holds a negative quantity
  * an intraday position squared off to exactly zero
"""
...
PRICE_DP = Decimal("0.0001")   # DECIMAL(18,4)
MONEY_DP = Decimal("0.01")     # DECIMAL(18,2)
...
BANK_ACCOUNTS = [
    # account_number,        name,                phone,          email,                       balance,     bank_name,        ifsc_code
...
# ---------------------------------------------------------------------------
# 020 clients  - covers ACTIVE / SUSPENDED / CLOSED
# ---------------------------------------------------------------------------

CLIENTS = [
    # client_id, account_number,          name,           email,                      phone,           status
...
# ---------------------------------------------------------------------------
# 030 auth  - bcrypt-shaped placeholder hashes, NOT real credentials
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 040 instruments  - instrument 8 stopped trading but keeps its row
# ---------------------------------------------------------------------------

INSTRUMENTS = [
    # instrument_id, instrument_name,        is_active, delisted_on
    (1, "RELIANCE",        "true",  ""),
    (2, "TCS",             "true",  ""),
    (3, "INFY",            "true",  ""),
    (4, "HDFCBANK",        "true",  ""),
...
# 050 orders
# ---------------------------------------------------------------------------
# (order_id, instrument_id, client_id, price_per_unit, side, product_type,
#  quantity, exchange, idempotency_key, status, failure_reason)
#
# status drives which downstream files the order appears in:
#   RECEIVED     -> orders only
#   IN_PROGRESS  -> orders + in_progress
#   SUCCESS      -> orders + transaction_success  (+ settles into a portfolio)
#   FAILED       -> orders + transaction_failures (settles nowhere)

ORDERS = [
    # --- client 1: delivery buys that build a holding, plus an intraday round trip
    ...
    # --- client 2: a delivery holding and an intraday SHORT left open
    ...
    # --- client 3: bigger book, one order still queued, one just received
    ...
    # --- client 4 is SUSPENDED: historical orders survive, nothing new settles
    ...
    # --- client 5: intraday squared off to exactly zero, plus a delisted instrument
    ...
    # --- client 6 is CLOSED: the row stays, so does its trading history
...
# ---------------------------------------------------------------------------
# settlement arithmetic - the reference implementation for the app layer
# ---------------------------------------------------------------------------

def apply_fill(qty, avg, side, fill_qty, fill_price):
    """
    Fold one fill into a (quantity, average price) position.

    Signed throughout: positive is long, negative is short. portfolio_holding
    only ever sees the long side (a delivery sell can reduce to zero but is
    never allowed to go past it); portfolio_positions can go either way.
    """
    signed = fill_qty if side == "BUY" else -fill_qty
    new_qty = qty + signed

    if qty == 0:
        new_avg = fill_price                      # opening a fresh position
    elif (qty > 0) == (signed > 0):
        # Adding to the same side: weighted average of the two legs.
        new_avg = (abs(qty) * avg + abs(signed) * fill_price) / abs(new_qty)
    elif new_qty == 0:
        new_avg = Decimal(0)                      # squared off flat
    elif (new_qty > 0) == (qty > 0):
        new_avg = avg                             # reduced, same side, cost basis unchanged
    else:
        new_avg = fill_price                      # flipped through zero
...
def settle_orders():
    """
    Replay every SUCCESS order into the portfolio table its product_type
    selects, and return both books keyed by (client_id, instrument_id).
    """
    holding = {}
    positions = {}
    ...
    for raw in ORDERS:
        o = order_dict(raw)
        if o["status"] != "SUCCESS":
            continue  # only a terminal SUCCESS moves the portfolio
...
def build_in_progress():
    """Only orders currently being worked sit in the queue."""
...
# Filename prefix is the LOAD ORDER; apply_db.py sorts on it and derives the
# table name from the rest.
BUILDERS = [
...
def render(header, rows):
    """CSV text with LF endings, so the files diff cleanly on every platform."""
